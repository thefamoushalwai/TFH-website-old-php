    ...
    <td style="width:100%;padding:12px;">
      ...
      <table cellspacing="0" cellpadding="0" style="width: 100%; margin-bottom:10px; padding:10px; border: solid 1px #666666; border-collapse:separate;">
        <tr>
          <td class="nobdr" style="width: 100%;">
            ...
            <strong>Service Location:</strong> <?php echo $varr['slocation']?>&nbsp;&nbsp;&nbsp;
            <strong>No of People:</strong> <?php echo $varr['noof_people']?><br>    

            <strong>Event Date:</strong> <?php echo $varr['event_date']?>&nbsp;&nbsp;&nbsp;
            <strong>Your Event:</strong> <?php echo $occasionsArr['occasions_title']?>
            </td>
        </tr>
      </table>
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="desc-table" style="width:100%; margin-bottom:3px;font-size: 12px;">
        <tr style="background-color: #135884; color: #fff !important; font-size: 14px; font-weight: 400;">
          ...
          <th align="center" style="width: 15%;padding: 3px;color:#fff">Image</th>
          <th align="center" style="width: 30%;padding: 3px;color:#fff">Menu Name</th>
          <th align="center" style="width: 22%;padding: 3px;color:#fff">Cuisine</th>
          <th align="center" style="width: 10%;padding: 3px;color:#fff">Food Type</th>          
          <th align="center" style="width: 15%;padding: 3px;color:#fff">Price (Rs.)</th>
        </tr>
        <?php        
        $order_detail_qry = db_query("SELECT * FROM order_detail WHERE order_members_slno = '".$varr['slno']."' && qryType='1' ");
        $slno=1; $total_cart_price =0;
        while($orderArr = db_fetch_array($order_detail_qry)) { 
          $menuArr = db_fetch_assoc(db_query("SELECT * from menu_item_tbl WHERE slno=".$orderArr['menu_item_slno'].""));         
          $cuisineArr = db_fetch_assoc(db_query("SELECT * FROM event_cuisine WHERE slno=".$orderArr['event_cuisine_slno']." "));

          if(!empty($menuArr['menu_img'])) {
            $imageName = "https://www.thefamoushalwai.com/frontEnd/menuimg/".$menuArr['menu_img']; 
          }
          else {
          ...
          ?>
          <tr>
          <td style="width: 8%;"><?php echo $slno?></td>
          <td style="width: 15%;"> <img src="<?php echo $imageName;?>" style=" border-radius: 4px;width: 50px;height: 30px;"></td>
          <td style="width: 30%;"><?php echo $menuArr['menu_name']?></td>
          <td style="width: 22%;"><?php echo $cuisineArr['cuisine_title']?></td>
        ...
        ?>
        <tr>
          <td colspan="5" style="background-color: #eee;text-align: right"><strong>IGST @18%</strong></td>
          <td style="text-align: center;background-color: #eee;">Rs.:<b><?php echo $gstAmt?></b></td>
          </tr>

        <tr>
          <td colspan="5" style="background-color: #eee;"><strong>Total in Words:</strong> <?php echo getIndianCurrency($total_paid)?> Only</td>
