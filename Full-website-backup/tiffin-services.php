...
$metatitle = 'Healthy Tiffin Meals Delivered | The Famous Halwai';
$metaDesc = 'Enjoy home-style tiffin meals delivered fresh daily. Simple, tasty, and affordable food made with care.';
$metaKeywords = 'tiffin service online, home style tiffin delivery, healthy tiffin meals, tiffin service near me';

$headTitle="Tiffin Services";
...
if($_POST['pID']=='AddPostForm') {
  ...
  if(!empty($_POST['mobile_phone'])) {
    ...
    if(!empty($_POST['meals'])) {
    	$your_query = "Meals: ".explode(",",$_POST['meals']);
    }
    if(!empty($_POST['cuisine'])) {
    	$your_query .= "Cuisine: ".explode(",",$_POST['cuisine']);
    }

    //echo "INSERT INTO general_inq SET query_for='Tiffin Services', contact_name = '".db_real_escape($_POST['contact_name'])."', email = '".$_POST['email']."', mobile_phone = '".$_POST['mobile_phone']."', state = '".$_POST['state']."', city = '".db_real_escape($_POST['city'])."', address = '".db_real_escape($_POST['address'])."', ipaddress='".$ip_address."', recv_date_time='".date("Y-m-d g:i:s")."', mem_add_from='4', no_of_people='".$_POST['no_of_people']."', call_phone_time='".$_POST['call_phone_time']."', your_query='".$your_query."'<br><br>";

    db_query("INSERT INTO general_inq SET query_for='Tiffin Services', contact_name = '".db_real_escape($_POST['contact_name'])."', email = '".$_POST['email']."', mobile_phone = '".$_POST['mobile_phone']."', state = '".$_POST['state']."', city = '".db_real_escape($_POST['city'])."', address = '".db_real_escape($_POST['address'])."', ipaddress='".$ip_address."', recv_date_time='".date("Y-m-d g:i:s")."', mem_add_from='4', no_of_people='".$_POST['no_of_people']."', call_phone_time='".$_POST['call_phone_time']."', your_query='".$your_query."' ");
    ?>
    <script type="text/javascript">
    //window.location.href='<?php echo SITE_URL;?>/tiffin-services.php?success_msg=Y';
    </script>
    <?php
  }
}
...
<!-- <div class="col-xl-12">	
<h1 class="serv_head">Tiffin Services</h1>
</div> -->
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">  
	    ...
	    <form name="postReqForm" id="postReqForm" method="post" action="<?php echo $_SERVER['PHP_SELF']?>" onsubmit="return validate_contact_form();">
	      <div class="regst-left">
	        <p class="">We are providing best Tiffin Services near in your location. Please fillup below form.</p>
	        <?php
	        if(isset($_REQUEST['success_msg']) && $_REQUEST['success_msg']=='Y') {
	          ?>
	          <div class="text-center mt-3 text-success"><h2>Thank you for interesting a Tiffin Services.<br>Our Representative will call you soon.</h2></div>
	          <?php
	        }
	        else {	        
	        	?>
	            <div class="form-steps-content mt-5">	              
	              ...
	              <div class="row">
	                ...
	                <label for="country">Select Meals<span class="text-danger">*</span></label>
	                <select class="floating-select form-control meals inputfild" id="meals" name="meals[]">
	                ...
	                <?php
                   	...
                   	while($mealsArr = db_fetch_assoc($meals_qry)) {
                    	?>
                    	<option value="<?php echo $mealsArr['slno']?>"><?php echo $mealsArr['meal_title']?></option>
                    	<?php
                    }
                    ?>	                
	                </select>                        
	                ...
	                <div class="form-group col-md-6">
	                <label for="country">Select Cuisine<span class="text-danger">*</span> <a href="<?php echo SITE_URL;?>/our-menu.php" target="_blank"><small>Our Menu</small></a></label>
	                <select class="floating-select form-control cuisine inputfild" id="cuisine" name="cuisine[]">
	                <option value="">-- Select Cuisine--</option>
	                <?php
                   	$cuisine_qry = db_query("SELECT * FROM event_cuisine WHERE slno IN (1,2,3)");
                   	while($cuisineArr = db_fetch_assoc($cuisine_qry)) {
                    	?>
                    	<option value="<?php echo $cuisineArr['slno']?>"><?php echo $cuisineArr['cuisine_title']?></option>
                    	<?php
                    }
                    ?>	                
	                </select>                        
	                ...
	                <div class="form-group col-md-6">
	                <label for="yname">Service Time <span class="text-danger">*</span></label>
	                <input type="text" class="form-control call_phone_time inputfild" id="call_phone_time" placeholder="Enter call_phone_time Time" name="service_time">
	                <span id="yname-info" class="error-info reg_error_msg"></span>
	                </div>

	                <div class="form-group col-md-6">
	                <label for="yname">No. of People <span class="text-danger">*</span></label>
	                <select name="programe_date" class="form-control inputfild no_of_people" id="no_of_people">
	                ...
	                <label for="city">Delivery Address <span class="text-danger">*</span></label>
	                <textarea type="text" class="form-control address inputfild" id="address" placeholder="Enter Your Address" name="address"></textarea>
...
function validate_contact_form() { 
   ...
   var valid = true;      
    ...
    if(!$(".meals").val()) {
        $(".meals").css('background-color','#f4e0e0');          
        $(".meals").css('border-color','#ee4e4e');
        $(".meals").css('border','1px solid #e11f26');
        valid = false;
    }
    if(!$(".cuisine").val()) {
        $(".cuisine").css('background-color','#f4e0e0');          
        $(".cuisine").css('border-color','#ee4e4e');
        $(".cuisine").css('border','1px solid #e11f26');
        valid = false;
    }    
