...
$metatitle='Our Items - The Famous Halwai';
$metaDesc='';
$metaKeywords='';
...
if($_REQUEST['part']=='submitMenuCart') {    
  ...
  foreach ($_SESSION["item_cart_item"] as $key => $value) {
      ...
      db_query("INSERT INTO order_detail SET order_members_slno = '".$mid."', event_cuisine_slno ='".$menuArr['event_cuisine_slno']."', menu_item_slno='".$value."', menu_rate='".$menuArr['menu_rate']."', state_slno='".$prodinfo['state_slno']."', veg_type='".$prodinfo['veg_type']."', recv_date='".date("Y-m-d")."', recv_time='".date("g:i:s")."', qryType='3', category='".$menuArr['category']."' ");
  }  
  
  ?>
  <script type="text/javascript">
  window.location.href = "<?php echo SITE_URL;?>/view_items_cart.php?success=yes&slno=<?php echo $mid;?>";  
  </script>
  <?php
  exit;
} 
...
if(!empty($_REQUEST['success']) && $_REQUEST['success']=='yes') { 
  ...
  if(!empty($_SESSION["item_cart_item"])) {
      $cartVal = count($_SESSION["item_cart_item"]);
  }
  ?>
  <section id="menu-item" class="cartitemh">
      <div class="menu-container">
          <h4 class="menu-section"> Your Items
          <sup>(<span class="text-danger" id="cart-item" style="font-family: sans-serif;"><?php echo $cartVal;?></span>)</sup> Summary</h4>          
                   ...
                   <div class="col-12 col-md-5 col-lg-3 right-side-box">                    
                     ...
                     <form name="cartForm" id="cartForm" method="post" action="<?php echo $_SERVER['PHP_SELF']?>" onsubmit="return post_menucart_validation();">    
                      <div class="ps-shopping__box">
                          ...
                          <input type="submit" name="submit" id="ProceedNow" class="btn btn-success v-btn"> <b>Pay Price:</b> Rs.<span class="text-danger"><b><?php echo $totalPay;?></b></span>
                          </div>
                          <div><a href="<?php echo SITE_URL;?>/chutney_achhar.php"><small class="add-more">Add More Items</small></a></div>
                      </div>
                      </form>
                  </div>
              </div>
