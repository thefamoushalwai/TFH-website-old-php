...
$metatitle ="Tiffin Services Inquiry";
if($_REQUEST['aID']=='updstatus') {
  ...
  if(isset($_POST['uID']) && $_POST['uID']=='editStatus') {  
    ...
    <div class="text-center mb-3"><h4><b>Update Status of Tiffin Services Inq</b></h4></div>      
      ...
      <div class="col-sm-6 text-right">
        ...
        foreach ($inq_status_arr as $key => $value) {
            if($_REQUEST['status']=="$key") {
                ?>
                <a href="<?php echo ADMIN_SITE_URL ?>/tiffin_services_inq.php?status=<?php echo $key;?>"><span class="btn-default" style="border-radius: 3px;background-color:#d9e7de;color:#181617;padding: 5px;color:#e11f26;"><b><?php echo $value;?></b></span></a>&nbsp;&nbsp;
                <?php
            }
            else {
                ?>
                <a href="<?php echo ADMIN_SITE_URL ?>/tiffin_services_inq.php?status=<?php echo $key;?>"><span class="btn-default" style="border-radius: 3px;background-color:#d9e7de;color:#181617;padding: 5px;"><?php echo $value;?></span></a>&nbsp;&nbsp;
                <?php
            }
        }
        ?>
...
<div class="col-md-12 table-responsive overflow-x1">
  <table class="table table-bordered mb-1">
    ...
    <?php
    $general_inquiry_qry = db_query("SELECT * FROM general_inq WHERE query_for='Tiffin Services' && status='".$_REQUEST['status']."' ");
    ...
    if(db_num_rows($general_inquiry_qry)>0) {
      while($ordArr = db_fetch_assoc($general_inquiry_qry)) {
        ...
        <tr>
          ...
          <a href="javascript:multiple_openwin('<?php echo ADMIN_SITE_URL ?>/tiffin_services_inq.php?aID=updstatus&slno=<?php echo $ordArr['slno']?>','700','400','props');" class="btn-outline-warning"><i class="fa fa-edit nav-icon"></i></a>
          </td>
          ...
          <?php echo $ordArr['your_query']?><br>
          </td>          

          <td class="nowrap_space">          
          Call Time Time: <?php echo $ordArr['call_phone_time']?>
          </td>          
