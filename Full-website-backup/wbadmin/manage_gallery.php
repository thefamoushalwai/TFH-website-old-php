    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6 text-right">
         <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active"><a href="<?php echo ADMIN_SITE_URL ?>/addedit_gallery.php"><span class="btn-info" style="border-radius: 3px;padding: 5px;">Add Gallery Image</span></a></li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
...
<section class="section profile"> 
  ...
  if(!empty($_REQUEST['success']) && $_REQUEST['success']=='yes') {  
    ?>
    <div class="text-success text-center mt-3"><h3>Gallery Image has updated successfully.</h3></div><br>
    <?php
  }       
  ...
  if(db_num_rows($winfo_qry)>0) {
    ...
    <tr class="tr-text-white">         
      ...
      <th class="nowrap_space" width="20%">Image Title</th>
      <th class="nowrap_space" width="20%">Images</th>            
      ...
      while($carr = db_fetch_assoc($winfo_qry)) {
        ...
        <tr class="ui-state-default" data-index="<?php echo $carr['slno']?>" data-position="<?php echo $carr['position']?>">
          ...
          <td><?php echo ucwords(strtolower($carr['image_title']));?></td>

          <td>
           <a href="<?php echo SITE_URL;?>/frontEnd/gallery/<?php echo $carr['gimage']?>" target="_blank"><img src="<?php echo SITE_URL;?>/frontEnd/gallery/<?php echo $carr['gimage']?>" width="120" height="120"></a>
          </td>
       
        
          <td>
          <a href="addedit_gallery.php?slno=<?php echo $carr['slno']?>" alt="Edit Gallery Image" title="Edit Gallery Image"><button type="button" class="btn btn-primary btn-sm p-1" style="vertical-align:top;"> <i class="bi bi-pencil"></i> Edit</button></a>  
          <?php
          if($carr['status']=='Y') {
            ?>&nbsp;&nbsp;
      ...
      ?>
      <div class="text-center text-danger" style="font-size:16px;">No Any Gallery Images Added</div>
      <?php
    }
    ?>
  </div>       
