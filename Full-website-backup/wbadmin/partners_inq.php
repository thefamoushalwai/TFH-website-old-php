      ...
      <div class="col-sm-6 text-right">
        ...
        foreach ($inq_status_arr as $key => $value) {
          if($_REQUEST['status']=="$key") {
              ?>
              <a href="<?php echo ADMIN_SITE_URL ?>/partners_inq.php?status=<?php echo $key;?>"><span class="btn-default" style="border-radius: 3px;background-color:#d9e7de;color:#181617;padding: 5px;color:#e11f26;"><b><?php echo $value;?></b></span></a>&nbsp;&nbsp;
              <?php
          }
          else {
              ?>
              <a href="<?php echo ADMIN_SITE_URL ?>/partners_inq.php?status=<?php echo $key;?>"><span class="btn-default" style="border-radius: 3px;background-color:#d9e7de;color:#181617;padding: 5px;"><?php echo $value;?></span></a>&nbsp;&nbsp;
              <?php
          }
        }
        ?>        
...
<div class="col-md-12 table-responsive overflow-x">
  <table class="table table-bordered mb-1">
    ...
    <?php
    $general_inquiry_qry = db_query("SELECT * FROM general_inq WHERE query_for='New Partner' && status='".$_REQUEST['status']."' ORDER BY slno DESC ");
    ...
    if(db_num_rows($general_inquiry_qry)>0) {
      while($ordArr = db_fetch_assoc($general_inquiry_qry)) {
        ...
        <tr>
          ...
          <td class="nowrap_space" style="width: 20%"><?php echo $ordArr['contact_name']?><br>
          ...
          <td class="nowrap_space"><?php echo $ordArr['address']?>, <?php echo $ordArr['city']?>, <?php echo $state_name_arr[$ordArr['state']]?><br>           
            
          </td>
          <td class="nowrap_space">
          Partner Type: <?php echo $ordArr['partner_type']?><br>  
          <?php
          if($ordArr['referralcode'])  {
            ?>
            <b>Referral Code:</b> <?php echo $ordArr['referralcode']?>
            <?php
          }
          ?> 
          </td>          
