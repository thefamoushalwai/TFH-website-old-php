    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6 text-right">
         <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active"><a href="<?php echo ADMIN_SITE_URL ?>/addedit_events.php"><span class="btn-info" style="border-radius: 3px;padding: 5px;">Add New Events</span></a></li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
...
<section class="section profile">
  ...
  if(!empty($_REQUEST['success']) && $_REQUEST['success']=='yes') {  
    ?>
    <div class="text-success text-center mt-3"><h3>Event Detail has updated successfully.</h3></div><br>
    <?php
  }        
   
  $occasions_qry = db_query("SELECT * FROM event_requirement WHERE 1=1 order by position ASC");        
  if(db_num_rows($occasions_qry)>0) {
    ...
    <th class="nowrap_space" width="20%">Event Name</th>      
    <th class="nowrap_space" width="20%">Event Image</th>      
    ...
    <th class="nowrap_space" width="20%">Action</th>
      ...
      while($carr = db_fetch_assoc($occasions_qry)) {
        ...
        <tr class="ui-state-default" data-index="<?php echo $carr['slno']?>" data-position="<?php echo $carr['position']?>">
          ...
          <td>
          <?php echo ucwords(strtolower($carr['event_title']));?>
          </td>

          <td>
          <?php
          if(!empty($carr['event_img'])) {
            ?>&nbsp;&nbsp;&nbsp;&nbsp;
            <a href="<?php echo SITE_URL;?>/frontEnd/event/<?php echo $carr['event_img']?>" target="_blank">View</a>
            <?php
          }
          ?>  
          </td>
          ...
          <td>
          <a href="addedit_events.php?slno=<?php echo $carr['slno']?>" alt="Edit Occasions" title="Edit Occasions"><button type="button" class="btn btn-primary btn-sm p-1" style="vertical-align:top;"> <i class="bi bi-pencil"></i> Edit</button></a>  
          <?php
