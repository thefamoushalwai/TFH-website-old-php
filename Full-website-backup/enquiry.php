...
$metatitle='Find Halwai Near Me & Book Halwai & Chefs Online | The Famous Halwai';
$metaDesc='Looking for a halwai near me or expert chefs? Book trusted halwais and chefs online with The Famous Halwai for weddings, poojas, parties, events, and catering services at your location.';
$metaKeywords='halwai near me, online chef, halwai for home, book halwai online, wedding catering, party halwai, event chefs';



//db_query("ALTER TABLE `order_inquiry` ADD `jobworker_slno` INT(5) NOT NULL AFTER `gas_burners`, ADD `nohalwaichef` INT(5) NOT NULL AFTER `jobworker_slno`;");

include('inner_header.php');
function days_in_month($month, $year){
    // calculate number of days in a monthYour Nam
    return $month == 2 ? ($year % 4 ? 28 : ($year % 100 ? 29 : ($year % 400 ? 28 : 29))) : (($month - 1) % 7 % 2 ? 30 : 31);
}

if(!empty($_REQUEST['success']) && $_REQUEST['success']=='yes') { 
  ?>
  <div class="text-center mt-5" style="height: 200px;margin-top: 110px!important;">
...
.enq-head {background: url("<?php echo SITE_URL;?>/frontEnd/images/hbanner/inquiry_header.jpg")no-repeat center center;background-size: cover;height: 250px;}
...
<h1 class="h1title">Enquiry Now</h1>	
	...
	if($_REQUEST['part']=='Submit_Enuiry_Form') {
	  ...
	  db_query("INSERT INTO order_inquiry SET slocation='".$_POST['slocation']."', occasions_slno='".$_POST['occasions_slno']."', email='".$_POST['email']."', noof_people='".$_POST['noof_people']."', mobile_phone='".$_POST['mobileno']."', occasions_date='".$_POST['occasions_date']."', contact_name='".$_POST['yname']."', starters='".$_POST['starters_val']."', main_cousrse='".$_POST['main_cousrse_val']."', dessert='".$_POST['dessert_slno_val']."', bread_rice_raita='".$_POST['brr_slno_val']."',  soups_beverages='".$_POST['soupb_slno_val']."', live_barbecue='".$_POST['barbecue_slno_val']."', breakfast_val='".$_POST['breakfast_val']."', lunch_val='".$_POST['lunch_val']."', evening_snaks='".$_POST['evening_snaks']."', dinner='".$_POST['dinner']."', enquiryType='".$_POST['enquiryType']."', cusine='".$_POST['cusine']."', gas_burners='".$_POST['gas_burners']."', ipAddress='".$ip_address."', created='".date("Y-m-d h:i:s")."', jobworker_slno='".$_POST['jobworker_slno']."', nohalwaichef='".$_POST['nohalwaichef']."'  ");
	...
	?>
	<form name="searchfrm" id="searchfrm" class="formstep1" action="<?php echo $_SERVER['PHP_SELF']?>" method="post" data-parsley-validate autocomplete="off" enctype="multipart/form-data">
		...
		if(empty($_REQUEST['step'])) {
		    ...
		    <div class="row">
		        ...
		        <div class="col-sm-11 col-xs-12">
		          <div class="x_panel">          
		                  ...
		                  <?php	
						    ...
						    for ($pdt=$cmonth; $pdt <=$cmonth; $pdt++) {
						    	...
						    	for($i=$cdays;$i<=$daysinMOnth;$i++) {
						        	...
						        	$years = date('Y', strtotime($getDate));
						        	$daysSelect = $day_num."-".$month_name."-".$years."-".$day_name;
						        	$daysSelect_display = $day_num."&nbsp;&nbsp;".$month_name."&nbsp;&nbsp;".$years."&nbsp;&nbsp;".$day_name;
						    ...
						    for ($pdt=$cmonth+1; $pdt <=$totalY; $pdt++) {
							  	$view_month = date($pdt);  $view_year = date("Y");
						    	$daysinMOnth = days_in_month($view_month, $view_year); 
		            ...
		            <?php
		            $cuisine_qry = db_query("SELECT * FROM event_cuisine WHERE slno NOT IN ('5','9','8')");
	        ...
	        <button type="button" class="submit_link_btn" onclick="proceesBtn_Step1(event);" style="width: 236px;">Process Now</button>
	        </div>
		    </div>
		    <?php
		}
			...
			<div class="col-sm-10"> 
			<select name="noof_people" class="form-control noof_people" id="programe_date1" required>
			<option value="">-- Select One--</option>
			<?php
			...
			<div class="form-group row">
			<div class="col-sm-12"> <label for="title_en" class="form-control-label"> Starters(Required)</label></div>
			<div class="col-sm-10"> 				
				<input type="hidden" id="starters_val" name="starters_val">	
			<select class="form-control starters" id="starters" required>
			<?php
            ...
            <div class="form-group row">
              <div class="col-sm-12"> <label for="title_en" class="form-control-label"> Breads, Rice and Raita(Required)</label></div>
              <div class="col-sm-10"> 
            ...
            <div class="form-group row">
              <div class="col-sm-12"> <label for="title_en" class="form-control-label"> Desserts(Required)</label></div> 
              <div class="col-sm-10"> 
              	<input type="hidden" id="dessert_slno_val" name="dessert_slno_val" >
				<select class="form-control dessert_slno" id="dessert_slno" multiple required>
				<?php
            ...
            <div class="form-group row">
            <div class="col-sm-12">   <label for="title_en" class="form-control-label"> Soups & Beverages(Required)</label></div> 
			<div class="col-sm-10">
            ...
            <div class="form-group row">
            <div class="col-sm-12">   <label for="title_en" class="form-control-label"> Live Barbecue(Required)</label></div> 
			<div class="col-sm-10"> 
            ...
            <?php			
			//if($_REQUEST['enquiryType']=='halwaichef') {
				?>
	            <div class="form-group row">
				<div class="col-sm-12"> <label for="title_en" class="form-control-label"><span>*</span> No. of Halwai/Chef(Required)</label> </div>
				<div class="col-sm-10"> 

				<div class="row">
				<div class="col-sm-6">		
				<select name="jobworker_slno" class="form-control jobworker_slno" id="jobworker_slno" required>
				<option value="">-- Select One--</option>
				<?php
				$jwrate_qry = db_query("SELECT * FROM job_worker_rate WHERE status='Y' ");				
				while($arr=db_fetch_assoc($jwrate_qry)) {
					?>
					<option value="<?php echo $arr['slno']?>"><?php echo $arr['jobworker']?></option>
					<?php 
				}
				?>
				</select>
				...
				<div class="col-sm-6">		
				<select name="nohalwaichef" class="form-control nohalwaichef" id="nohalwaichef" required>
				<option value="">-- Select One--</option>
				<?php
				for ($pcnt=1; $pcnt <=10; $pcnt++) { 
					?>
					<option value="<?php echo $pcnt?>"><?php echo $pcnt?></option>
					<?php 
				}
				?>
				</select>
				...
				<div class="form-group row">
				<div class="col-sm-12"> <label for="title_en" class="form-control-label">Selected No. of Halwai/Chef Rate</label><b> Rs.</b> <span class="display_rate text-danger"></span></div>
				</div>
				<?php
			//}
			?>		
	        ...
	        <button type="submit" class="submit_link_btn" onclick="proceesBtn_Step2(event);" style="width: 236px;">Submit Now</button>
	        </div>
		    </div>
	    	<?php
	    }
...
function proceesBtn_Step1(e) {
    e.preventDefault();
    if (document.querySelector('.formstep1').checkValidity()) {
        // Collect values
        var eventType = $("#req_slno").val();
        var pdate = $("#programe_date").val();
        var breakfast = $(".breakfast_val").val();
        ...
        var mobileno = $(".mobileno").val();
        var gas_burners = $(".gas_burners").val();
        var cuisines = $(".cuisines_val").val();

        window.location.href = "<?php echo SITE_URL;?>/enquiry.php?step=2" +
            "&et=" + encodeURIComponent(eventType) +
            "&pdt=" + encodeURIComponent(pdate) +
            "&breakfast=" + encodeURIComponent(breakfast) +
            "&lunch=" + encodeURIComponent(lunch) +
            "&evening=" + encodeURIComponent(evening) +
            "&dinner=" + encodeURIComponent(dinner) +
            "&enquiryType=" + encodeURIComponent(enquiryType) +
            "&slocation=" + encodeURIComponent(slocation) +
            "&yname=" + encodeURIComponent(yname) +
            "&email=" + encodeURIComponent(email) +
            "&mobileno=" + encodeURIComponent(mobileno) +
            "&gas_burners=" + encodeURIComponent(gas_burners) +
            "&cusine=" + encodeURIComponent(cuisines);
    } else {
        document.querySelector('.formstep1').reportValidity();
    }
}

// function proceesBtn_Step2(e) {
//     e.preventDefault();
//     const form = document.getElementById('searchfrm');

//     // Check native HTML validation
//     if (!form.checkValidity()) {
//         // Trigger native HTML5 validation messages
//         form.reportValidity();
//         return;
//     }

//     // Example: custom additional validation for multiple select (if needed)
//     var startersSelect = $("#starters_val").val();
//     if (startersSelect.trim()== ""){
//         alert('Please select at least one Starter.');
//         return;
//     }
...
function proceesBtn_Step2(e) {
    ...
    // Custom validation for Starters
    var startersSelect = $("#starters_val").val();
    if (startersSelect.trim() == "") {
        alert('Please select at least one Starter.');
        return;
    ...
    // Custom validation for Breads, Rice, and Raita
    var brrSelect = $("#brr_slno_val").val();
    if (brrSelect.trim() == "") {
        alert('Please select at least one item from Breads, Rice and Raita.');
        return;
    }
    
    // Custom validation for Desserts
    var dessertSelect = $("#dessert_slno_val").val();
     if (dessertSelect.trim() == "") {
        alert('Please select at least one dessert.');
        return;
    }
    
    // Custom validation for Soups & Beverages
    var soupbSelect = $("#soupb_slno_val").val();
     if (soupbSelect.trim() == "") {
        alert('Please select at least one item from Soups & Beverages.');
        return;
    }
   

    // Custom validation for Live Barbecue
    var barbecueSelect = $("#barbecue_slno_val").val();
    if (barbecueSelect.trim() == "") {
        alert('Please select at least one item from Live Barbecue.');
        return;
    }
...
$(".jobworker_slno,.nohalwaichef").change(function () {	
	var nohalwaichef= $(".nohalwaichef").val();
	var jobworker_slno= $(".jobworker_slno").val();
	jQuery.ajax({
		type:'POST',
		url:'ajaxjQuery.php',
		data:'slno='+jobworker_slno+'&nohalwaichef='+nohalwaichef+'&part=Get_Nohalwaichef_Rate',
		dataType:'html',
		/*beforeSend: function(){
		  	$('.display_rate').html('Please Wait.....');
		},*/
	  	success: function(responseData) {	  		
			$('.display_rate').html(responseData);			
	    }
	});
});
</script>
